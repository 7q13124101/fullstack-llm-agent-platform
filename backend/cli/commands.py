...
@click.group()
@click.version_option(version="0.1.0", prog_name="fullstack_llm_agent_platform")
def cli():
    """fullstack_llm_agent_platform management CLI."""
    pass
...
def user_create(email: str, password: str, role: str, superuser: bool):
    ...
    from app.services.user import UserService    from app.db.session import async_session_maker
    ...
    async def _create():
        async with async_session_maker() as session:
            ...
            try:
                ...
                return None    user = asyncio.run(_create())    if user:
...
def user_set_role(email: str, role: str):
    ...
    user = asyncio.run(_update())    if user:
