...
from fastapi.responses import JSONResponsefrom sqlalchemy import text
from app.core.config import settingsfrom app.api.deps import DBSession
router = APIRouter()
...
async def readiness_probe(    db: DBSession,) -> dict[str, Any] | JSONResponse:
    ...
    Checks performed:    - Database connectivity
    Returns:
...
async def readiness_check(    db: DBSession,) -> dict[str, Any] | JSONResponse:
    """Readiness check (alias for /health/ready).

    Deprecated: Use /health/ready instead.
    """
    return await readiness_probe(        db=db,    )
