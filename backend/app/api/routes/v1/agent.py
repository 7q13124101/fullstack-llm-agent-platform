...
import logging
from typing import Anyfrom datetime import datetime, UTCfrom uuid import UUID
from fastapi import APIRouter, WebSocket, WebSocketDisconnect
from langchain_core.messages import AIMessage, AIMessageChunk, HumanMessage, SystemMessage, ToolMessage

from app.agents.langgraph_assistant import AgentContext, get_agentfrom app.db.session import get_db_contextfrom app.api.deps import ConversationSvc, get_conversation_service
...
async def agent_websocket(
    ...
    context: AgentContext = {}    current_conversation_id: str | None = None
    try:
        while True:
            ...
            try:                async with get_db_context() as db:
                    ...
                    if requested_conv_id:
                        ...
                        conv_data = ConversationCreate(                            title=user_message[:50] if len(user_message) > 50 else user_message,
                        )
            ...
            try:
                ...
                if current_conversation_id and final_output:
                    try:                        async with get_db_context() as db:
                            ...
                            assistant_msg_id = str(assistant_msg.id)                    except Exception as e:
                        logger.warning(f"Failed to persist assistant response: {e}")
                # Notify frontend that assistant message was saved with real database ID                if assistant_msg_id:
