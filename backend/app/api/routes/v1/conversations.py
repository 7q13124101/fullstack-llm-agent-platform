...
from fastapi.responses import JSONResponsefrom app.api.deps import DBSession, ConversationSvcfrom app.api.deps import CurrentAdmin, CurrentUserfrom app.api.deps import MessageRatingSvcfrom app.schemas.conversation import (
    ConversationCreate,
    ConversationList,
    ConversationRead,
    ...
    MessageList,
    MessageRead,
    MessageReadSimple,    ConversationAdminList,)from app.schemas.message_rating import (
    MessageRatingCreate,
    MessageRatingRead,
)
router = APIRouter()
...
async def export_conversations(
    conversation_service: ConversationSvc,    current_user: CurrentAdmin,) -> Any:
    ...
    return JSONResponse(content={"conversations": export_data, "total": len(export_data)},
        headers={"Content-Disposition": 'attachment; filename="conversations_export.json"'})@router.get("/admin-list", response_model=ConversationAdminList)
...
async def list_conversations(
    conversation_service: ConversationSvc,    current_user: CurrentUser,    skip: int = Query(0, ge=0, description="Number of conversations to skip"),
    ...
    """
    items, total = await conversation_service.list_conversations(        user_id=current_user.id,        skip=skip,
        limit=limit,
        include_archived=include_archived,
    )
...
async def create_conversation(
    conversation_service: ConversationSvc,    current_user: CurrentUser,    data: ConversationCreate | None = None,
) -> Any:
    ...
    """
    if data is None:
        data = ConversationCreate()    data.user_id = current_user.id    return await conversation_service.create_conversation(data)


@router.get("/{conversation_id}", response_model=ConversationReadWithMessages)
async def get_conversation(
    conversation_id: UUID,
    conversation_service: ConversationSvc,    current_user: CurrentUser,) -> Any:
    """Get a conversation with all its messages.

    Raises 404 if the conversation does not exist.
    """
    return await conversation_service.get_conversation(
        conversation_id, include_messages=True,        user_id=current_user.id,    )
...
async def update_conversation(
    ...
    return await conversation_service.update_conversation(
        conversation_id, data,        user_id=current_user.id,    )


@router.delete("/{conversation_id}", status_code=status.HTTP_204_NO_CONTENT, response_model=None)
async def delete_conversation(
    conversation_id: UUID,
    conversation_service: ConversationSvc,    current_user: CurrentUser,) -> None:
...
async def list_messages(
    conversation_id: UUID,
    conversation_service: ConversationSvc,    current_user: CurrentUser,    skip: int = Query(0, ge=0),
    ...
    items, total = await conversation_service.list_messages(
        ...
        skip=skip,
        limit=limit,
        include_tool_calls=True,        user_id=current_user.id,    )
