...
from typing import Literalfrom uuid import UUID
...
class ConversationShareCreate(BaseSchema):
    """Schema for creating a conversation share."""    shared_with: UUID | None = Field(default=None, description="User ID to share with (omit for link sharing)")    permission: Literal["view", "edit"] = Field(default="view", description="Access level")
    generate_link: bool = Field(default=False, description="Generate a public share link")


class ConversationShareRead(BaseSchema):
    """Schema for reading a conversation share."""    id: UUID
    conversation_id: UUID
    shared_by: UUID
    shared_with: UUID | None = None    share_token: str | None = None
...
class AdminConversationRead(BaseSchema):
    """Admin view of a conversation — includes owner email."""    id: UUID
...
class AdminUserRead(BaseSchema):
    """Minimal user info for admin endpoints."""    id: UUID    email: str
