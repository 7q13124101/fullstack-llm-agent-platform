...
from sqlalchemy.orm import selectinloadfrom sqlalchemy import String
...
async def get_conversations_by_user(
    ...
    query = select(Conversation)    if user_id:
        query = query.where(Conversation.user_id == user_id)    if not include_archived:
    ...
    result = await db.execute(query)
    return list(result.scalars().all())async def get_all_conversations_with_count(
    db: AsyncSession,
    *,
    skip: int = 0,
    limit: int = 50,
...
async def count_conversations(
    ...
    query = select(func.count(Conversation.id))    if user_id:
        query = query.where(Conversation.user_id == user_id)    if not include_archived:
        query = query.where(Conversation.is_archived == False)  # noqa: E712
    result = await db.execute(query)
...
async def create_conversation(
    db: AsyncSession,
    *,    user_id: UUID | None = None,    title: str | None = None,
) -> Conversation:
...
async def get_messages_by_conversation(
    ...
    *,
    ...
    if include_tool_calls:
        query = query.options(selectinload(Message.tool_calls))    from app.db.models.chat_file import ChatFile
    query = query.options(selectinload(Message.files))    query = query.order_by(Message.created_at.asc()).offset(skip).limit(limit)
    result = await db.execute(query)
