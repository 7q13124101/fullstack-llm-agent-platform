...
from app.db.models.conversation import Conversation, Message, ToolCallfrom app.db.models.message_rating import MessageRating
from app.db.models.user import Userfrom app.repositories import conversation_repofrom app.repositories import conversation_share_repofrom app.schemas.conversation import (
    ConversationCreate,
    ConversationUpdate,
    MessageCreate,
    MessageRead,
    ToolCallCreate,
    ToolCallComplete,    ConversationWithLatestMessage,)
...
class ConversationService:
    ...
    async def export_all(self) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            for conv in items:
                ...
                all_message_ids.extend([m.id for m in messages if m.id])            # Fetch ratings for this chunk of messages
            ...
            for conv in items:
                ...
                export_data.append({
                    "id": str(conv.id),                    "user_id": str(conv.user_id) if conv.user_id else None,                    "title": conv.title,
                    ...
                    "messages": [{
                        ...
                        "tool_calls": [{
                            ...
                            for tc in (m.tool_calls or [])] if hasattr(m, "tool_calls") and m.tool_calls else [],                        "ratings": message_ratings_map.get(str(m.id), []),                    } for m in messages],
                })
    ...
    async def get_conversation(
        ...
        *,
        include_messages: bool = False,        user_id: UUID | None = None,    ) -> Conversation:
    ...
    async def list_conversations(
        ...
        items = await conversation_repo.get_conversations_by_user(
            self.db,            user_id=user_id,            skip=skip,
            limit=limit,
            include_archived=include_archived,
        )
        total = await conversation_repo.count_conversations(
            self.db,            user_id=user_id,            include_archived=include_archived,
        )
        return items, total    async def list_conversations_admin(
        self,
        *,
        skip: int = 0,
        limit: int = 50,
    ...
    async def create_conversation(
        ...
        return await conversation_repo.create_conversation(
            self.db,            user_id=data.user_id,            title=data.title,
        )

    async def update_conversation(
        self,
        conversation_id: UUID,
        data: ConversationUpdate,        user_id: UUID | None = None,    ) -> Conversation:
    ...
    async def archive_conversation(
        self,
        conversation_id: UUID,        user_id: UUID | None = None,    ) -> Conversation:
    ...
    async def delete_conversation(
        self,
        conversation_id: UUID,        user_id: UUID | None = None,    ) -> bool:
    ...
    async def list_messages(
        ...
        *,
        skip: int = 0,
        limit: int = 100,
        include_tool_calls: bool = False,        user_id: UUID | None = None,    ) -> tuple[list[Message | MessageRead], int]:
