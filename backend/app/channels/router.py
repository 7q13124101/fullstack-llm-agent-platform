...
class ChannelMessageRouter:
    ...
    async def _route_inner(self, incoming: IncomingMessage, db: Any) -> None:
        ...
        # 1. Load bot        bot = await channel_bot_repo.get_by_id(db, incoming.bot_id)        if not bot or not bot.is_active:
        ...
        # 7. Invoke agent
        tool_events = []
        try:            svc = AgentInvocationService(db)
    ...
    async def _handle_command(
        ...
        if cmd == "/new":
            from app.repositories import channel_session_repo, conversation_repo            session = await channel_session_repo.get_by_bot_and_chat(
        ...
        if cmd == "/link":
            ...
            try:                linked = await channel_identity_repo.get_by_link_code(db, arg)
        ...
        if cmd == "/unlink":
            from app.repositories import channel_identity_repo            identity = await channel_identity_repo.get_by_platform_user(
                db,
                platform=incoming.platform,
                platform_user_id=incoming.platform_user_id,
            )
    ...
    async def _resolve_identity(
        ...
        policy: dict[str, Any] = self._parse_policy(bot)
        mode: str = policy.get("mode", "open")        identity = await channel_identity_repo.get_by_platform_user(
            db,
            platform=incoming.platform,
            platform_user_id=incoming.platform_user_id,
        )
