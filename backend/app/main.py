...
from app.core.middleware import RequestIDMiddlewarefrom app.rag.embeddings import EmbeddingServicefrom app.rag.vectorstore import PgVectorStorefrom app.rag.vectorstore import BaseVectorStore

class LifespanState(TypedDict, total=False):
    """Lifespan state - resources available via request.state."""    embedding_service: EmbeddingService
...
async def lifespan(app: FastAPI) -> AsyncGenerator[LifespanState, None]:
    ...
    """
    # === Startup ===    state: LifespanState = {}    setup_logfire()    from app.core.logfire_setup import instrument_asyncpg
    instrument_asyncpg()    from app.core.config import settings
    try:
        embedder = EmbeddingService(settings=settings.rag)
        embedder.warmup()
        state["embedding_service"] = embedder
    except Exception as e:
        logger.error(f"Embedding service warmup failed: {e}. RAG will not be available.")    # Initialize and warmup reranker (downloads model or validates API key)
    try:
        ...
        logger.warning(f"Reranker warmup failed: {e}. Reranking will be disabled.")    if "embedding_service" in state:
        try:
    ...
    try:        from app.db.session import get_db_context
        async with get_db_context() as _db:
            from app.repositories.channel_bot import get_active_polling_bots
            _polling_bots = await get_active_polling_bots(_db, "telegram")        for _bot in _polling_bots:
        ...
        logger.error("Telegram: failed to start polling: %s", _exc)    yield state
    # === Shutdown ===    from app.db.session import close_db
    await close_db()    try:
        if "vector_store" in state:
            await state["vector_store"].engine.dispose()  # type: ignore[attr-defined]
    except Exception:
        pass    for _bid in list(_telegram_adapter._polling_tasks.keys()):
...
## Features- **Authentication**: JWT-based authentication with refresh tokens- **API Key**: Header-based API key authentication- **Database**: Async database operations- **Observability**: Logfire integration for tracing and monitoring- **RAG**: Retrieval Augmented Generation with Milvus and LangChain
## Documentation
...
- [ReDoc](/redoc) - Alternative documentation view
        ...
        contact={
            "name": "7q13124101",
            "email": "7q13124101@mail.tsust.edu.tw",
        },
        license_info={
            "name": "MIT",
            "identifier": "MIT",
    ...
    # Admin panel (environment restricted)    ADMIN_ALLOWED_ENVIRONMENTS = ["development", "local", "staging"]
