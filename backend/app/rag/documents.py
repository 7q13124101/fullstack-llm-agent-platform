...
from langchain_text_splitters import RecursiveCharacterTextSplitterimport pymupdf
...
class DocumentProcessor:
    ...
    3. Chunk document pages using RecursiveCharacterTextSplitter    Supported file types:
    ...
    def __init__(self, settings: RAGSettings):
        ...
        self.text_parser = TextDocumentParser()        self.docx_parser = DocxDocumentParser()        self.pdf_parser = PyMuPDFParser(enable_ocr=False)
    @staticmethod
    ...
    async def process_file(self, filepath: Path) -> Document:
        ...
        if filepath.suffix in (".txt", ".md"):
            document = await self.text_parser.parse(filepath)        elif filepath.suffix == ".docx":
            document = await self.docx_parser.parse(filepath)
        elif filepath.suffix == ".pdf":
            document = await self.pdf_parser.parse(filepath)        else:
