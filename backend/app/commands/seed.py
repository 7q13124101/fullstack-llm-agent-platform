...
@click.option("--dry-run", is_flag=True, help="Show what would be created without making changes")@click.option("--users/--no-users", default=True, help="Seed users (default: True)")def seed(
    count: int,
    clear: bool,
    dry_run: bool,    users: bool,) -> None:
    """
    ...
    Example:
        ...
        project cmd seed --dry-run        project cmd seed --no-users  # Skip user seeding    """
    if not HAS_FAKER:
    ...
    if dry_run:
        ...
        if clear:
            info("[DRY RUN] Would clear existing data first")        if users:
            info("[DRY RUN] Would create users")        return    from app.db.session import async_session_maker    from app.db.models.user import User
    from app.core.security import get_password_hash
    async def _seed() -> None:
        async with async_session_maker() as session:
            created_counts = {}            # Seed users
            if users:
                if clear:
