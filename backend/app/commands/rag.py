...
from app.rag.vectorstore import BaseVectorStorefrom app.rag.vectorstore import PgVectorStore
...
def get_rag_services() -> tuple[RAGSettings, BaseVectorStore, DocumentProcessor, RetrievalService, IngestionService]:
    ...
    embedder = EmbeddingService(settings=settings)    vector_store = PgVectorStore(settings=settings, embedding_service=embedder)    processor = DocumentProcessor(settings=settings)
...
async def ingest_path_async(
    ...
    import hashlib    from datetime import UTC, datetime

    from app.db.models.rag_document import RAGDocument
    from app.db.models.sync_log import SyncLog
    from app.db.session import get_db_context    from tqdm import tqdm
    ...
    error_count = 0
    replaced_count = 0
    skipped_count = 0    # Create SyncLog
            ...
            try:
                ...
                if result.status.value == "done":
                    ...
                    if result.message and "replaced" in result.message:
                        replaced_count += 1                    async with get_db_context() as db:
                    ...
                    error_count += 1
                    tqdm.write(f"  ✗ {filepath.name}: {result.error_message}")                    async with get_db_context() as db:
                ...
                error_count += 1
                tqdm.write(f"  ✗ {filepath.name}: {str(e)}")                async with get_db_context() as db:
                    rag_doc_result = await db.get(RAGDocument, doc_id)
                    if rag_doc_result:
                        rag_doc_result.status = "error"
                        rag_doc_result.error_message = str(e)
                        rag_doc_result.completed_at = datetime.now(UTC)
                        await db.commit()    # Update SyncLog
    async with get_db_context() as db:
...
def rag_sources() -> None:
    """List all configured sync sources with their status."""    from app.db.session import get_db_context
...
def rag_source_remove(source_id: str, yes: bool) -> None:
    ...
    """
    if not yes:
        click.confirm(f"Are you sure you want to remove sync source '{source_id}'?", abort=True)    from app.db.session import get_db_context
...
def rag_source_sync(source_id: str | None, sync_all: bool) -> None:
    ...
    if not source_id and not sync_all:
        ...
        return    from app.db.session import get_db_context
