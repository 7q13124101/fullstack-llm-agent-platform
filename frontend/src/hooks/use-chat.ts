...
import { WS_URL } from "@/lib/constants";import { useConversationStore } from "@/stores";interface UseChatOptions {
...
export function useChat(options: UseChatOptions = {}) {
  ...
  const { setCurrentConversationId, currentConversationId: currentConversationIdFromStore } = useConversationStore();  const {
    messages,
    addMessage,
    updateMessage,
    addToolCall,
  ...
  const handleWebSocketMessage = useCallback(
    (event: MessageEvent) => {
      ...
      const createNewMessage = (content: string): string => {
        ...
        const newMsgId = nanoid();        // Use current conversationId from store to avoid closure issues
        const effectiveConversationId = currentConversationIdFromStore || conversationId || undefined;        addMessage({
          id: newMsgId,
          role: "assistant",
          content,
          timestamp: new Date(),
          isStreaming: true,
          toolCalls: [],
          groupId: currentGroupIdRef.current || undefined,          conversationId: effectiveConversationId,
          isTemporaryId: true,        });
        setCurrentMessageId(newMsgId);
        return newMsgId;
      };

      switch (wsEvent.type) {        case "conversation_created": {
  ...
  const doSend = useCallback(
    (content: string, fileIds?: string[]) => {
      addMessage({
        ...
        role: "user",
        content,
        timestamp: new Date(),        conversationId: conversationId || undefined,        fileIds,
      });
      setIsProcessing(true);      const payload: Record<string, unknown> = {
        message: content,
        conversation_id: conversationId || null,
      };
      if (fileIds?.length) payload.file_ids = fileIds;
      if (modelRef.current) payload.model = modelRef.current;
      sendMessage(payload);    },    [addMessage, sendMessage, conversationId]  );
