...
import { CopyButton } from "./copy-button";import { RatingButtons } from "./rating-buttons";
import { useChatStore } from "@/stores";import { User, Bot } from "lucide-react";
...
export function MessageItem({ message, groupPosition }: MessageItemProps) {
  const isUser = message.role === "user";  const updateMessage = useChatStore((state) => state.updateMessage);  const { user: authUser } = useAuthStore();
