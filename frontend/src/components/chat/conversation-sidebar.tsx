...
import type { Conversation } from "@/types";import { ShareDialog } from "./share-dialog";
interface ConversationItemProps {
  ...
  onDelete: () => void;
  onArchive: () => void;
  onRename: (title: string) => void;  onShare: () => void;}
...
function ConversationItem({
  ...
  onDelete,
  onArchive,
  onRename,  onShare,}: ConversationItemProps) {
...
function ConversationList({
  ...
  const activeConversations = (conversations ?? []).filter((c) => !c.is_archived);  const [shareConversationId, setShareConversationId] = useState<string | null>(null);
          ...
          <div className="space-y-1">
            {activeConversations.map((conversation) => (
              <ConversationItem
                ...
                onRename={(title) => onRename(conversation.id, title)}                onShare={() => setShareConversationId(conversation.id)}              />
            ))}
          </div>
        )}
      </div>      {shareConversationId && (
        <ShareDialog
