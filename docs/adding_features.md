...
Run with: `uv run fullstack_llm_agent_platform cmd my-command --name test`
...
# Or use CLI
uv run fullstack_llm_agent_platform db migrate -m "Add notifications table"
uv run fullstack_llm_agent_platform db upgrade
```