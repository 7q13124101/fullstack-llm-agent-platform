...
### Embedding ProvidersEmbeddings are generated using **Google Gemini** (`gemini-embedding-exp-03-07`
by default). Supports multimodal embeddings (text + images).
### Vector StorageVectors are stored in **pgvector** using the existing PostgreSQL database.
...
reranking by passing `use_reranker=True` to the search API.Reranking uses a cross-encoder model (`CROSS_ENCODER_MODEL`, default:
