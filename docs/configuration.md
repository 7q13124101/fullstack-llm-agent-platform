...
|----------|---------|-------------|
| `PROJECT_NAME` | `fullstack_llm_agent_platform` | Display name for the project |
...
| `POSTGRES_DB` | `fullstack_llm_agent_platform` | Database name |
...
| `LOGFIRE_SERVICE_NAME` | `fullstack_llm_agent_platform` | Service name in Logfire dashboard |
...
| `LANGCHAIN_PROJECT` | `fullstack_llm_agent_platform` | Project name in LangSmith |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CHANNEL_ENCRYPTION_KEY` | (empty) | Fernet key for encrypting bot tokens. Generate with: `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` || `TELEGRAM_WEBHOOK_BASE_URL` | (empty) | Base URL for Telegram webhook (e.g. `https://yourdomain.com`). Required only in webhook mode |
## CORS
...
Before deploying to production, ensure these variables are properly set:1. `SECRET_KEY` -- Generate a unique 64-character hex key: `openssl rand -hex 32`2. `API_KEY` -- Generate a unique key: `openssl rand -hex 32`3. `ENVIRONMENT` -- Set to `production`
4. `DEBUG` -- Set to `false`5. `POSTGRES_PASSWORD` -- Use a strong, unique password6. `CORS_ORIGINS` -- List only your actual frontend domain(s)8. `GOOGLE_API_KEY` -- Your production API key